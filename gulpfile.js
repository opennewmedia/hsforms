...
gulp.task('ts:compile', function(){
  return tsProject.src()
    .pipe(tsProject())
    .js.pipe(gulp.dest(jsPath+'/ts'));
});

// JAVASCRIPT COMPRESSION
gulp.task('js:compressed', function(done){
  gulp.src([jsPath + '/Src/*.js', '!'+ jsPath +'/*.min.js'])
    .pipe(minify({
        builtIns: false,
        evaluate: false,
        mangle: false
...
function copyVendorScripts(done) {
  gulp.src(['node_modules/jquery/dist/*.js', 'node_modules/jquery/dist/*.map'], { buffer: false })
      .pipe(gulp.dest('Resources/Public/build/vendor/jquery'));
  gulp.src(['node_modules/moment/min/*.min.js'], { buffer: false })
      .pipe(gulp.dest('Resources/Public/build/vendor/moment'));
  gulp.src(['node_modules/cal/build/js/*.min.js', 'node_modules/cal/build/js/*/*.map'], { buffer: false })
      .pipe(gulp.dest('Resources/Public/build/vendor/cal/js'));
  gulp.src(['node_modules/cal/build/css/*.min.css', 'node_modules/cal/build/css/maps/*.map'], { buffer: false })
      .pipe(gulp.dest('Resources/Public/build/vendor/cal/css'));
  gulp.src(['node_modules/toastr/build/*.min.css'], { buffer: false })
      .pipe(gulp.dest('Resources/Public/build/vendor/toastr/css'));
  gulp.src(['node_modules/toastr/build/*.min.js', 'node_modules/toastr/build/*.js.map'], { buffer: false })
      .pipe(gulp.dest('Resources/Public/build/vendor/toastr/js'));
  gulp.src(['node_modules/jquery-modal/*.min.js', 'node_modules/jquery-modal/*.min.css'], { buffer: false })
      .pipe(gulp.dest('Resources/Public/build/vendor/jquery-modal'));
  // it needs to be minified as well
  gulp.src(['node_modules/responsive-toolkit/dist/*toolkit.min.js'])
      .pipe(gulp.dest('Resources/Public/build/vendor/responsive-toolkit/'));
  done();
}
...
gulp.task('build', gulp.series(gulp.parallel('sass:uncompressed', 'sass:compressed'), copyVendorScripts));
