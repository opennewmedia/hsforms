...
class HotelsuiteApi {
    ...
    public function __construct() 
    {
        ...
        $this->settings = $extbaseFrameworkConfiguration['plugin.']['tx_hsforms_form.']['settings.'];
        
        // check if s8key has to be overridden via TS
        if($this->settings['config.']['s8key']) {
            $this->extensionConfiguration['s8key'] = $this->settings['config.']['s8key'];
        }
    }
    ...
    public function fetchData($endpoint = FALSE, $accessToken = FALSE)
    {
        ...
        if($this->extensionConfiguration['s8key']) {
           $headers[] = 'X-S8Key: ' . $this->extensionConfiguration['s8key'];
        }
    ...
    public function getCurrentAccessToken() 
    {
        ...
        $row = GeneralUtility::makeInstance(ConnectionPool::class)
                ->getConnectionForTable('tx_hsforms_config')
                ->select(
                    ['config', 'value'],                // fields to select
                    'tx_hsforms_config',               // from
    ...
    public function updateAccessToken() 
    {
        ...
        if ($this->accessToken) {
            ...
            GeneralUtility::makeInstance(ConnectionPool::class)->getConnectionForTable('tx_hsforms_config')
            ->delete(
                'tx_hsforms_config',          // from
                [ 'config' => 'access_token' ] // where
            );
    ...
    public function getNewAccessToken() 
    {
        ...
        $url = $this->extensionConfiguration['apiUrl'] . '/oauth';
